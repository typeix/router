...
describe("Parser", () => {
  ...
  test("parse and createUrl", () => {
    let route = "/can<any>one/<name:\\w+>/should<now:\\W+>do-it/<see:(\\w+)>-<nice:([a-zA-Z]+)>-now-<only:\\d+>-not/user/<id:\\d+>";
    let parser = new RouteParser(route);
    expect(parser.createUrl({
      "any": "1454zfhg=?`='(    ()=(",
      "name": "igor",
      "now": "#+",
      "see": "whata",
      ...
      "only": "2306",
      "id": "1412"
    })).toBe("/can1454zfhg=?`='(    ()=(one/igor/should#+do-it/whata-smile-now-2306-not/user/1412");
  });
  ...
  test("parse and check params", () => {
    ...
    expect(parser.createUrl(p1)).toEqual(url1);
    ...
    expect(parser.createUrl(p2)).toEqual(url2);
  });
  ...
  test("Should test patterns on /<category:(.*)>/page/<pageNum:(.*)>", () => {
    ...
    expect(pattern.createUrl(params)).toBe("/category/value/page/1/abc");
    ...
    expect(pattern.createUrl(params1)).toBe("/category/page/1");
  });
  ...
  test("Should get correct parameters on /assets/<file:(.*)>", () => {
    ...
    expect(pattern.createUrl(params)).toEqual(url);
  });
